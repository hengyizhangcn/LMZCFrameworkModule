...
#import "CSCAddressModel.h"

typedef NS_ENUM(NSInteger, CSCAddressPickerMode) {
    // 只显示省
    CSCAddressPickerModeProvince = 1,
    // 显示省市
    CSCAddressPickerModeCity,
    // 显示省市区（默认）
    CSCAddressPickerModeArea
};

typedef void(^CSCAddressResultBlock)(CSCProvinceModel *province, CSCCityModel *city, CSCAreaModel *area);
typedef void(^CSCAddressCancelBlock)(void);

@interface CSCAddressPickerView : CSCBaseView
