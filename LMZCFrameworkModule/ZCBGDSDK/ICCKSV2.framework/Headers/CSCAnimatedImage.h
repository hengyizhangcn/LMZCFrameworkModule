...
/**
 This is the protocol for CSCAnimatedImage class only but not for CSCAnimatedImageCoder. If you want to provide a custom animated image class with full advanced function, you can conform to this instead of the base protocol.
 */
@protocol CSCAnimatedImage <CSCAnimatedImageProvider>
 ...
 */
- (nullable instancetype)initWithData:(nonnull NSData *)data scale:(CGFloat)scale options:(nullable CSCImageCoderOptions *)options;

/**
 Initializes the image with an animated coder. You can use the coder to decode the image frame later.
 @note We use this with animated coder which conforms to `CSCProgressiveImageCoder` for progressive animation decoding.
 
 @param animatedCoder An animated coder which conform `CSCAnimatedImageCoder` protocol
 @param scale The scale factor to assume when interpreting the image data. Applying a scale factor of 1.0 results in an image whose size matches the pixel-based dimensions of the image. Applying a different scale factor changes the size of the image as reported by the `size` property.
 @return An initialized object
 */
- (nullable instancetype)initWithAnimatedCoder:(nonnull id<CSCAnimatedImageCoder>)animatedCoder scale:(CGFloat)scale;
...
/**
 The image class which supports animating on `CSCAnimatedImageView`. You can also use it on normal UIImageView/NSImageView.
 */
@interface CSCAnimatedImage : UIImage <CSCAnimatedImage>
 ...
 */
@property (nonatomic, assign, readonly) CSCImageFormat animatedImageFormat;
...
// By default, animated image frames are returned by decoding just in time without keeping into memory. But you can choose to preload them into memory as well, See the decsription in `CSCAnimatedImage` protocol.
