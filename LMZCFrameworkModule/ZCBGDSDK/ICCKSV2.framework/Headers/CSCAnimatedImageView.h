...
/**
 ...
 Call `setImage:` with `UIImage(NSImage)` which conform to `CSCAnimatedImage` protocol will start animated image rendering. Call with normal UIImage(NSImage) will back to normal UIImageView(NSImageView) rendering
 ...
 */
@interface CSCAnimatedImageView : UIImageView
...
/**
 ...
 @note The best practice to use incremental load is using `initWithAnimatedCoder:scale:` in `CSCAnimatedImage` with animated coder which conform to `CSCProgressiveImageCoder` as well. Then call incremental update and incremental decode method to produce the image.
...
/**
 ...
 Default is NSRunLoopCommonModes on multi-core iOS device, NCSCefaultRunLoopMode on single-core iOS device
 */
