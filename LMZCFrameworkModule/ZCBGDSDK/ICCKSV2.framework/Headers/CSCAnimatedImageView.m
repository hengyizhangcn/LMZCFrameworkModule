...
static NSUInteger CSCDeviceTotalMemory() {
    return (NSUInteger)[[NSProcessInfo processInfo] physicalMemory];
}

static NSUInteger CSCDeviceFreeMemory() {
...
@interface CSCAnimatedImageView () <CALayerDelegate> {
...
@property (nonatomic, strong) UIImage<CSCAnimatedImage> *animatedImage;
...
@implementation CSCAnimatedImageView
#if CSC_UIKIT
...
+ (instancetype)imageViewWithImage:(NSImage *)image
{
    ...
    CSCAnimatedImageView *imageView = [[CSCAnimatedImageView alloc] initWithFrame:frame];
...
- (void)setImage:(UIImage *)image
{
    ...
    if ([image.class conformsToProtocol:@protocol(CSCAnimatedImage)]) {
        NSUInteger animatedImageFrameCount = ((UIImage<CSCAnimatedImage> *)image).animatedImageFrameCount;
        ...
        self.animatedImage = (UIImage<CSCAnimatedImage> *)image;
...
- (CADisplayLink *)displayLink
{
    if (!_displayLink) {
        ...
        CSCWeakProxy *weakProxy = [CSCWeakProxy proxyWithTarget:self];
...
- (void)stopAnimating
{
    if (self.animatedImage) {
        [_fetchQueue cancelAllOperations];
        // Using `_displayLink` here because when UIImageView dealloc, it may trigger `[self stopAnimating]`, we already release the display link in CSCAnimatedImageView's dealloc method.
#if CSC_MAC
...
- (void)updateIsProgressiveWithImage:(UIImage *)image
{
    ...
    if ([image.class conformsToProtocol:@protocol(CSCAnimatedImage)] && image.sd_isIncremental) {
        UIImage *previousImage = self.image;
        if ([previousImage.class conformsToProtocol:@protocol(CSCAnimatedImage)] && previousImage.sd_isIncremental) {
            NSData *previousData = [((UIImage<CSCAnimatedImage> *)previousImage) animatedImageData];
            NSData *currentData = [((UIImage<CSCAnimatedImage> *)image) animatedImageData];
            ...
            // Warning: normally the `previousData` is same instance as `currentData` because our `CSCAnimatedImage` class share the same `coder` instance internally. But there may be a race condition, that later retrived `currentData` is already been updated and it's not the same instance as `previousData`.
            // And for protocol extensible design, we should not assume `CSCAnimatedImage` protocol implementations always share same instance. So both of two reasons, we need that `rangeOfData` check.
...
#pragma clang diagnostic pop
    ...
    if (!fetchFrame && !bufferFull && self.fetchQueue.operationCount == 0) {
        ...
        UIImage<CSCAnimatedImage> *animatedImage = self.animatedImage;
        @weakify(self);
...
- (void)calculateMaxBufferCount {
    ...
    if (self.maxBufferSize > 0) {
        ...
        NSUInteger total = CSCDeviceTotalMemory();
        NSUInteger free = CSCDeviceFreeMemory();
...
static CVReturn DisplayLinkCallback(CVDisplayLinkRef displayLink, const CVTimeStamp *inNow, const CVTimeStamp *inOutputTime, CVOptionFlags flagsIn, CVOptionFlags *flagsOut, void *displayLinkContext) {
    ...
    CSCAnimatedImageView *imageView = (__bridge CSCAnimatedImageView *)displayLinkContext;
    __weak CSCAnimatedImageView *weakImageView = imageView;
    dispatch_async(dispatch_get_main_queue(), ^{
