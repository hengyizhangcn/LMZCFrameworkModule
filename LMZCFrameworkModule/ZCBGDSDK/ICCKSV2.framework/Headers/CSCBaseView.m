...
- (UIView *)alertView {
    if (!_alertView) {
        _alertView = [[UIView alloc]initWithFrame:CGRectMake(0, SCREEN_HEIGHT - kTopViewHeight - kPickerHeight - CSC_BOTTOM_MARGIN, SCREEN_WIDTH, kTopViewHeight + kPickerHeight + CSC_BOTTOM_MARGIN)];
...
- (UIView *)topView {
    if (!_topView) {
        ...
        _topView.backgroundColor = kCSCToolBarColor;
        // 设置子视图的大小随着父视图变化
...
- (UIButton *)leftBtn {
    if (!_leftBtn) {
        ...
        _leftBtn.backgroundColor = kCSCToolBarColor;
...
- (UIButton *)rightBtn {
    if (!_rightBtn) {
        ...
        _rightBtn.backgroundColor = kCSCToolBarColor;
...
- (UIView *)lineView {
    if (!_lineView) {
        ...
        _lineView.backgroundColor = CSC_RGB_HEX(0xf1f1f1, 1.0f);
