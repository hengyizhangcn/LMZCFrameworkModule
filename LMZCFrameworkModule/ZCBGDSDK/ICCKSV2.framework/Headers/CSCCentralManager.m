...
#import "CSCCentralManager.h"
#import "CSCCallback.h"



@implementation CSCCentralManager

#define currChannel [cscSpeaker callbackOnCurrChannel]
...
- (instancetype)init {
    ...
    if (self) {
        ...
        NSDictionary *options = [NSDictionary dictionaryWithObjectsAndKeys:
                                 ...
                                 @"CSCBluetoothRestore",CBCentralManagerOptionRestoreIdentifierKey,
                                 nil];
...
- (void)scanPeripherals {
    [centralManager scanForPeripheralsWithServices:[currChannel CSCOptions].scanForPeripheralsWithServices options:[currChannel CSCOptions].scanForPeripheralsWithOptions];
}

//连接Peripherals
- (void)connectToPeripheral:(CBPeripheral *)peripheral{
    [centralManager connectPeripheral:peripheral options:[currChannel CSCOptions].connectPeripheralWithOptions];
}
...
- (void)centralManagerDidUpdateState:(CBCentralManager *)central {
    ...
    //发送通知
    [[NSNotificationCenter defaultCenter]postNotificationName:CSCNotificationAtCentralManagerDidUpdateState object:@{@"central":central}];
    
    switch (central.state) {
        case CBCentralManagerStateUnknown:
            CSCLog(@">>>CBCentralManagerStateUnknown");
            break;
        case CBCentralManagerStateResetting:
            CSCLog(@">>>CBCentralManagerStateResetting");
            break;
        case CBCentralManagerStateUnsupported:
            CSCLog(@">>>CBCentralManagerStateUnsupported");
            break;
        case CBCentralManagerStateUnauthorized:
            CSCLog(@">>>CBCentralManagerStateUnauthorized");
            break;
        case CBCentralManagerStatePoweredOff:
            CSCLog(@">>>CBCentralManagerStatePoweredOff");
            break;
        case CBCentralManagerStatePoweredOn:
            CSCLog(@">>>CBCentralManagerStatePoweredOn");
            [[NSNotificationCenter defaultCenter]postNotificationName:CSCNotificationAtCentralManagerEnable object:@{@"central":central}];
            break;
        default:
            break;
    }
...
- (void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI {
    ...
    //日志
    //CSCLog(@"当扫描到设备:%@",peripheral.name);
    [self addDiscoverPeripheral:peripheral];
    
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:CSCNotificationAtDidDiscoverPeripheral
    ...
    if ([currChannel filterOnDiscoverPeripherals]) {
        if ([currChannel filterOnDiscoverPeripherals](peripheral.name,advertisementData,RSSI)) {
            if ([currChannel blockOnDiscoverPeripherals]) {
                [[cscSpeaker callbackOnCurrChannel] blockOnDiscoverPeripherals](central,peripheral,advertisementData,RSSI);
            }
        }
    }
...
- (void)centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral {
    ...
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:CSCNotificationAtDidConnectPeripheral
                                                       object:@{@"central":central,@"peripheral":peripheral}];
    
    //设置委托
    [peripheral setDelegate:self];
    
    //CSCLog(@">>>连接到名称为（%@）的设备-成功",peripheral.name);
    ...
    if (needDiscoverServices) {
        [peripheral discoverServices:[currChannel CSCOptions].discoverWithServices];
...
- (void)centralManager:(CBCentralManager *)central didFailToConnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error {
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:CSCNotificationAtDidFailToConnectPeripheral
                                                       object:@{@"central":central,@"peripheral":peripheral,@"error":error?error:@""}];
 
    //    CSCLog(@">>>连接到名称为（%@）的设备-失败,原因:%@",[peripheral name],[error localizedDescription]);
...
- (void)centralManager:(CBCentralManager *)central didDisconnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error {
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:CSCNotificationAtDidDisconnectPeripheral
                                                       object:@{@"central":central,@"peripheral":peripheral,@"error":error?error:@""}];
    
    //    CSCLog(@">>>外设连接断开连接 %@: %@\n", [peripheral name], [error localizedDescription]);
    if (error)
    {
        CSCLog(@">>> didDisconnectPeripheral for %@ with error: %@", peripheral.name, [error localizedDescription]);
    }
    ...
    if ([self findConnectedPeripherals].count == 0) {
        ...
        //    CSCLog(@">>> stopConnectAllPerihperals");
    }
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverServices:(NSError *)error {
    ...
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:CSCNotificationAtDidDiscoverServices
                                                       object:@{@"peripheral":peripheral,@"error":error?error:@""}];
    
    //  CSCLog(@">>>扫描到服务：%@",peripheral.services);
    if (error) {
        CSCLog(@">>>didDiscoverServices for %@ with error: %@", peripheral.name, [error localizedDescription]);
    }
    //回叫block
    ...
    if (needDiscoverCharacteristics) {
        for (CBService *service in peripheral.services) {
            [peripheral discoverCharacteristics:[currChannel CSCOptions].discoverWithCharacteristics forService:service];
        }
    }
}
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error {
    ...
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:CSCNotificationAtDidDiscoverCharacteristicsForService
                                                       object:@{@"peripheral":peripheral,@"service":service,@"error":error?error:@""}];
    
    
    if (error) {
        CSCLog(@"error didDiscoverCharacteristicsForService for %@ with error: %@", service.UUID, [error localizedDescription]);
        //        return;
    }
    //回叫block
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:CSCNotificationAtDidUpdateValueForCharacteristic
                                                       object:@{@"peripheral":peripheral,@"characteristic":characteristic,@"error":error?error:@""}];
    
    if (error) {
        CSCLog(@"error didUpdateValueForCharacteristic %@ with error: %@", characteristic.UUID, [error localizedDescription]);
        //        return;
    }
    //查找字段订阅
    if ([cscSpeaker notifyCallback:characteristic]) {
        [cscSpeaker notifyCallback:characteristic](peripheral,characteristic,error);
        return;
    }
    //回叫block
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverDescriptorsForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    if (error) {
        CSCLog(@"error Discovered DescriptorsForCharacteristic for %@ with error: %@", characteristic.UUID, [error localizedDescription]);
        //        return;
    }
    //回叫block
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForDescriptor:(CBDescriptor *)descriptor error:(NSError *)error {
    ...
    if (error) {
        CSCLog(@"error didUpdateValueForDescriptor  for %@ with error: %@", descriptor.UUID, [error localizedDescription]);
        //        return;
    }
    //回叫block
...
- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    [[NSNotificationCenter defaultCenter]postNotificationName:CSCNotificationAtDidWriteValueForCharacteristic object:@{@"characteristic":characteristic,@"error":error?error:@""}];
    
    //    CSCLog(@">>>didWriteValueForCharacteristic");
    //    CSCLog(@">>>uuid:%@,new value:%@",characteristic.UUID,characteristic.value);
...
- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForDescriptor:(CBDescriptor *)descriptor error:(NSError *)error {
    //    CSCLog(@">>>didWriteValueForCharacteristic");
    //    CSCLog(@">>>uuid:%@,new value:%@",descriptor.UUID,descriptor.value);
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateNotificationStateForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    [[NSNotificationCenter defaultCenter]postNotificationName:CSCNotificationAtDidUpdateNotificationStateForCharacteristic object:@{@"characteristic":characteristic,@"error":error?error:@""}];
    
    CSCLog(@">>>didUpdateNotificationStateForCharacteristic");
    CSCLog(@">>>uuid:%@,isNotifying:%@",characteristic.UUID,characteristic.isNotifying?@"isNotifying":@"Notifying");
...
- (void)peripheralDidUpdateRSSI:(CBPeripheral *)peripheral error:(nullable NSError *)error {
    [[NSNotificationCenter defaultCenter]postNotificationName:CSCNotificationAtDidReadRSSI object:@{@"peripheral":peripheral,@"RSSI":peripheral.RSSI,@"error":error?error:@""}];
    
    CSCLog(@">>>peripheralDidUpdateRSSI -> RSSI:%@",peripheral.RSSI);
...
- (void)peripheral:(CBPeripheral *)peripheral didReadRSSI:(NSNumber *)RSSI error:(NSError *)error {
    [[NSNotificationCenter defaultCenter]postNotificationName:CSCNotificationAtDidReadRSSI object:@{@"peripheral":peripheral,@"RSSI":RSSI,@"error":error?error:@""}];
    
    CSCLog(@">>>peripheralDidUpdateRSSI -> RSSI:%@",RSSI);
    if ([currChannel blockOnDidReadRSSI]) {
