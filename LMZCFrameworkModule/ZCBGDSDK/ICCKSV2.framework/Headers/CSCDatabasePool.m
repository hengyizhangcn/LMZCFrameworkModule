...
- (void)pushDatabaseBackInPool:(CSCDatabase*)db;
- (CSCDatabase*)db;

@end


@implementation CSCDatabasePool
...
+ (instancetype)databasePoolWithURL:(NSURL *)url {
    return CSCDBReturnAutoreleased([[self alloc] initWithPath:url.path]);
}

+ (instancetype)databasePoolWithPath:(NSString *)aPath flags:(int)openFlags {
    return CSCDBReturnAutoreleased([[self alloc] initWithPath:aPath flags:openFlags]);
}

+ (instancetype)databasePoolWithURL:(NSURL *)url flags:(int)openFlags {
    return CSCDBReturnAutoreleased([[self alloc] initWithPath:url.path flags:openFlags]);
}
...
- (instancetype)initWithPath:(NSString*)aPath flags:(int)openFlags vfs:(NSString *)vfsName {
    ...
    if (self != nil) {
        ...
        _databaseInPool     = CSCDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = CSCDBReturnRetained([NSMutableArray array]);
...
- (void)dealloc {
    ...
    _delegate = 0x00;
    CSCDBRelease(_path);
    CSCDBRelease(_databaseInPool);
    CSCDBRelease(_databaseOutPool);
    CSCDBRelease(_vfsName);
    
    if (_lockQueue) {
        CSCDBDispatchQueueRelease(_lockQueue);
        _lockQueue = 0x00;
    }
...
- (void)pushDatabaseBackInPool:(CSCDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The CSCDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
