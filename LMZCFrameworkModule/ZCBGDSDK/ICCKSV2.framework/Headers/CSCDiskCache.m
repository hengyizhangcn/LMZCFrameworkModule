...
#pragma mark - CSCcachePathForKeyDiskCache Protocol
- (instancetype)initWithCachePath:(NSString *)cachePath config:(nonnull CSCImageCacheConfig *)config {
    if (self = [super init]) {
...
- (BOOL)containsDataForKey:(NSString *)key {
    ...
    // fallback because of https://github.com/rs/CSCWebImage/pull/976 that added the extension to the disk file name
...
- (void)removeExpiredData {
    ...
    switch (self.config.diskCacheExpireType) {
        case CSCImageCacheConfigExpireTypeAccessDate:
            cacheContentDateKey = NSURLContentAccessDateKey;
            break;
            
        case CSCImageCacheConfigExpireTypeModificationDate:
...
- (nullable NSString *)cachePathForKey:(nullable NSString *)key inPath:(nonnull NSString *)path {
    NSString *filename = CSCDiskCacheFileNameForKey(key);
...
#define CSC_MAX_FILE_EXTENSION_LENGTH (NAME_MAX - CC_MD5_DIGEST_LENGTH * 2 - 1)

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
static inline NSString * _Nonnull CSCDiskCacheFileNameForKey(NSString * _Nullable key) {
    ...
    if (ext.length > CSC_MAX_FILE_EXTENSION_LENGTH) {
        ext = nil;
    }
