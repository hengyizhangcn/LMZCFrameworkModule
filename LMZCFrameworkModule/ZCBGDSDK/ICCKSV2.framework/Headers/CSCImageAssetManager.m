...
static NSArray *CSCBundlePreferredScales() {
...
@implementation CSCImageAssetManager {
    dispatch_semaphore_t _lock;
}

+ (instancetype)sharedAssetManager {
    static dispatch_once_t onceToken;
    static CSCImageAssetManager *assetManager;
    dispatch_once(&onceToken, ^{
        assetManager = [[CSCImageAssetManager alloc] init];
    });
    return assetManager;
}
...
- (NSString *)getPathForName:(NSString *)name bundle:(NSBundle *)bundle preferredScale:(CGFloat *)scale {
    ...
    NSArray *scales = CSCBundlePreferredScales();
