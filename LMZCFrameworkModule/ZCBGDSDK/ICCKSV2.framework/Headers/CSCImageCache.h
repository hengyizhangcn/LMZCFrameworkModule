...
/// Image Cache Options
typedef NS_OPTIONS(NSUInteger, CSCImageCacheOptions) {
    /**
     * By default, we do not query image data when the image is already cached in memory. This mask can force to query image data at the same time. However, this query is asynchronously unless you specify `CSCImageCacheQueryMemoryDataSync`
     */
    CSCImageCacheQueryMemoryData = 1 << 0,
    /**
     * By default, when you only specify `CSCImageCacheQueryMemoryData`, we query the memory image data asynchronously. Combined this mask as well to query the memory image data synchronously.
     */
    CSCImageCacheQueryMemoryDataSync = 1 << 1,
    /**
     ...
     */
    CSCImageCacheQueryDiskDataSync = 1 << 2,
    /**
     * By default, images are decoded respecting their original size. On iOS, this flag will scale down the
     * images to a size compatible with the constrained memory of devices.
     */
    CSCImageCacheScaleDownLargeImages = 1 << 3,
    /**
     * By default, we will decode the image in the background during cache query and download from the network. This can help to improve performance because when rendering image on the screen, it need to be firstly decoded. But this happen on the main queue by Core Animation.
     * However, this process may increase the memory usage as well. If you are experiencing a issue due to excessive memory consumption, This flag can prevent decode the image.
     */
    CSCImageCacheAvoidDecodeImage = 1 << 4,
    /**
     * By default, we decode the animated image. This flag can force decode the first frame only and produece the static image.
     */
    CSCImageCacheDecodeFirstFrameOnly = 1 << 5,
    /**
     * By default, for `CSCAnimatedImage`, we decode the animated image frame during rendering to reduce memory usage. This flag actually trigger `preloadAllAnimatedImageFrames = YES` after image load from disk cache
     */
    CSCImageCachePreloadAllFrames = 1 << 6,
    /**
     ...
     * Note this options is not compatible with `CSCImageCacheDecodeFirstFrameOnly`, which always produce a UIImage/NSImage.
     */
    CSCImageCacheMatchAnimatedImageClass = 1 << 7,
};

/**
 * CSCImageCache maintains a memory cache and a disk cache. Disk cache write operations are performed
 * asynchronous so it doesn’t add unnecessary latency to the UI.
 */
@interface CSCImageCache : NSObject
 ...
 */
@property (nonatomic, copy, nonnull, readonly) CSCImageCacheConfig *config;

/**
 * The memory cache implementation object used for current image cache.
 * By default we use `CSCMemoryCache` class, you can also use this to call your own implementation class method.
 * @note To customize this class, check `CSCImageCacheConfig.memoryCacheClass` property.
 */
@property (nonatomic, strong, readonly, nonnull) id<CSCMemoryCache> memoryCache;

/**
 * The disk cache implementation object used for current image cache.
 * By default we use `CSCMemoryCache` class, you can also use this to call your own implementation class method.
 * @note To customize this class, check `CSCImageCacheConfig.diskCacheClass` property.
 ...
 */
@property (nonatomic, strong, readonly, nonnull) id<CSCDiskCache> diskCache;
 ...
 */
@property (nonatomic, copy, nullable) CSCImageCacheAdditionalCachePathBlock additionalCachePathBlock;
 ...
 */
@property (nonatomic, class, readonly, nonnull) CSCImageCache *sharedImageCache;
 ...
 * If you don't provide the disk cache directory, we will use the User Cache directory with prefix (~/Library/Caches/com.hackemist.CSCImageCache/).
 *
 * @param ns        The namespace to use for this cache store
 ...
 * The final disk cache directory should looks like ($directory/$namespace). And the default config of shared cache, should result in (~/Library/Caches/com.hackemist.CSCImageCache/default/)
 *
 * @param ns          The namespace to use for this cache store
 * @param directory   Directory to cache disk images in
...
- (nonnull instancetype)initWithNamespace:(nonnull NSString *)ns
                       diskCacheDirectory:(nullable NSString *)directory
                                   config:(nullable CSCImageCacheConfig *)config NS_DESIGNATED_INITIALIZER;
 ...
 */
- (nullable NSOperation *)queryCacheOperationForKey:(nullable NSString *)key done:(nullable CSCImageCacheQueryCompletionBlock)doneBlock;
 ...
 * @return a NSOperation instance containing the cache op
 */
- (nullable NSOperation *)queryCacheOperationForKey:(nullable NSString *)key options:(CSCImageCacheOptions)options done:(nullable CSCImageCacheQueryCompletionBlock)doneBlock;
...
/**
 * CSCImageCache is the built-in image cache implementation for web image manager. It adopts `CSCImageCache` protocol to provide the function for web image manager to use for image loading process.
 */
@interface CSCImageCache (CSCImageCache) <CSCImageCache>
