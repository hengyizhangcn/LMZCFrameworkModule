...
typedef NS_ENUM(NSUInteger, CSCImageCacheConfigExpireType) {
    /**
     * When the image is accessed it will update this value
     */
    CSCImageCacheConfigExpireTypeAccessDate,
    /**
     * The image was obtained from the disk cache (Default)
     */
    CSCImageCacheConfigExpireTypeModificationDate
};
 ...
 */
@interface CSCImageCacheConfig : NSObject <NSCopying>

/**
 Gets the default cache config used for shared instance or initialization when it does not provide any cache config. Such as `CSCImageCache.sharedImageCache`.
 ...
 */
@property (nonatomic, class, readonly, nonnull) CSCImageCacheConfig *defaultCacheConfig;
...
/*
 * The option to control weak memory cache for images. When enable, `CSCImageCache`'s memory cache will use a weak maptable to store the image at the same time when it stored to memory, and get removed at the same time.
 ...
 */
@property (assign, nonatomic) CSCImageCacheConfigExpireType diskCacheExpireType;
...
/**
 * The custom memory cache class. Provided class instance must conform to `CSCMemoryCache` protocol to allow usage.
 * Defaults to built-in `CSCMemoryCache` class.
 * @note This value does not support dynamic changes. Which means further modification on this value after cache initlized has no effect.
 */
@property (assign, nonatomic, nonnull) Class memoryCacheClass;

/**
 * The custom disk cache class. Provided class instance must conform to `CSCDiskCache` protocol to allow usage.
 * Defaults to built-in `CSCDiskCache` class.
 * @note This value does not support dynamic changes. Which means further modification on this value after cache initlized has no effect.
 */
