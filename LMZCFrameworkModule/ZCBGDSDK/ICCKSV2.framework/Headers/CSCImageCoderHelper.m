...
#import "UIImage+CSCForceDecode.h"
...
/*
 * Defines the maximum size in MB of the decoded image when the flag `CSCWebImageScaleDownLargeImages` is set
...
/*
 * Defines the maximum size in MB of a tile used to decode image when the flag `CSCWebImageScaleDownLargeImages` is set
...
@implementation CSCImageCoderHelper

+ (UIImage *)animatedImageWithFrames:(NSArray<CSCImageFrame *> *)frames {
    ...
    [frames enumerateObjectsUsingBlock:^(CSCImageFrame * _Nonnull frame, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    CFStringRef imageUTType = [NSData sd_UTTypeFromImageFormat:CSCImageFormatGIF];
    ...
    CSCAnimatedImageRep *imageRep = [[CSCAnimatedImageRep alloc] initWithData:imageData];
...
+ (NSArray<CSCImageFrame *> *)framesFromAnimatedImage:(UIImage *)animatedImage {
    if (!animatedImage) {
        return nil;
    }
    
    NSMutableArray<CSCImageFrame *> *frames = [NSMutableArray array];
    ...
    [animatedImages enumerateObjectsUsingBlock:^(UIImage * _Nonnull image, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([image isEqual:previousImage]) {
            ...
            CSCImageFrame *frame = [CSCImageFrame frameWithImage:previousImage duration:avgDuration * repeatCount];
    ...
    for (size_t i = 0; i < frameCount; i++) {
        @autoreleasepool {
            ...
            CSCImageFrame *frame = [CSCImageFrame frameWithImage:frameImage duration:frameDuration];
            [frames addObject:frame];
        }
    }
#endif
...
+ (CGImageRef)CGImageCreateDecoded:(CGImageRef)cgImage orientation:(CGImagePropertyOrientation)orientation {
    ...
    // Apply transform
    CGAffineTransform transform = CSCCGContextTransformFromOrientation(orientation, CGSizeMake(newWidth, newHeight));
...
+ (UIImage *)decodedAndScaledDownImageWithImage:(UIImage *)image limitBytes:(NSUInteger)bytes {
    ...
    // on iOS7, do not forget to call [[CSCImageCache sharedImageCache] clearMemory];
    @autoreleasepool {
...
static inline CGAffineTransform CSCCGContextTransformFromOrientation(CGImagePropertyOrientation orientation, CGSize size) {
