...
@interface CSCImageIOCoderFrame : NSObject
...
@implementation CSCImageIOCoderFrame
@end

@implementation CSCImageIOAnimatedCoder {
    ...
    NSArray<CSCImageIOCoderFrame *> *_frames;
    BOOL _finished;
}
...
+ (CSCImageFormat)imageFormat {
    @throw [NSException exceptionWithName:NSInternalInconsistencyException
                                   reason:[NSString stringWithFormat:@"For `CSCImageIOAnimatedCoder` subclass, you must override %@ method", NSStringFromSelector(_cmd)]
                                 userInfo:nil];
}
...
- (UIImage *)decodedImageWithData:(NSData *)data options:(nullable CSCImageCoderOptions *)options {
    ...
    BOOL decodeFirstFrame = [options[CSCImageCoderDecodeFirstFrameOnly] boolValue];
    if (decodeFirstFrame || count <= 1) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            CSCImageFrame *frame = [CSCImageFrame frameWithImage:image duration:duration];
            [frames addObject:frame];
        }
        
        NSUInteger loopCount = [self.class imageLoopCountWithSource:source];
        
        animatedImage = [CSCImageCoderHelper animatedImageWithFrames:frames];
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(CSCImageFormat)format options:(nullable CSCImageCoderOptions *)options {
    ...
    NSArray<CSCImageFrame *> *frames = [CSCImageCoderHelper framesFromAnimatedImage:image];
    ...
    BOOL encodeFirstFrame = [options[CSCImageCoderEncodeFirstFrameOnly] boolValue];
...
#pragma mark - CSCAnimatedImageCoder
...
- (BOOL)scanAndCheckFramesValidWithImageSource:(CGImageSourceRef)imageSource {
    ...
    NSMutableArray<CSCImageIOCoderFrame *> *frames = [NSMutableArray array];
    
    for (size_t i = 0; i < frameCount; i++) {
        CSCImageIOCoderFrame *frame = [[CSCImageIOCoderFrame alloc] init];
        frame.index = i;
...
- (UIImage *)animatedImageFrameAtIndex:(NSUInteger)index {
    ...
    CGImageRef newImageRef = [CSCImageCoderHelper CGImageCreateDecoded:imageRef];
    if (!newImageRef) {
