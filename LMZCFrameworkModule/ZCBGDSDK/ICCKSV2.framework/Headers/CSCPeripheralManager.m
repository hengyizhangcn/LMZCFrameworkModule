...
#import "CSCPeripheralManager.h"
#import "CSCDefine.h"
#import "CSCSpeaker.h"

#define callbackBlock(...) if ([[cscSpeaker callback] __VA_ARGS__])   [[cscSpeaker callback] __VA_ARGS__ ]

@implementation CSCPeripheralManager {
    ...
    return ^CSCPeripheralManager *() {
        ...
        if ([self canStartAdvertising]) {
            ...
            if (PERIPHERAL_MANAGER_INIT_WAIT_TIMES > 5) {
                CSCLog(@">>>error： 第%d次等待peripheralManager打开任然失败，请检查蓝牙设备是否可用",PERIPHERAL_MANAGER_INIT_WAIT_TIMES);
            }
            dispatch_time_t popTime = dispatch_time(DISPATCH_TIME_NOW, 3.0 * NSEC_PER_SEC);
            dispatch_after(popTime, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                self.startAdvertising();
            });
            CSCLog(@">>> 第%d次等待peripheralManager打开",PERIPHERAL_MANAGER_INIT_WAIT_TIMES);
        }
...
- (CSCPeripheralManager *(^)(NSArray *array))addServices {
    return ^CSCPeripheralManager*(NSArray *array) {
...
- (void)peripheralManagerDidUpdateState:(CBPeripheralManager *)peripheral {
    switch (peripheral.state) {
        case CBPeripheralManagerStateUnknown:
            CSCLog(@">>>CBPeripheralManagerStateUnknown");
            break;
        case CBPeripheralManagerStateResetting:
            CSCLog(@">>>CBPeripheralManagerStateResetting");
            break;
        case CBPeripheralManagerStateUnsupported:
            CSCLog(@">>>CBPeripheralManagerStateUnsupported");
            break;
        case CBPeripheralManagerStateUnauthorized:
            CSCLog(@">>>CBPeripheralManagerStateUnauthorized");
            break;
        case CBPeripheralManagerStatePoweredOff:
            CSCLog(@">>>CBPeripheralManagerStatePoweredOff");
            break;
        case CBPeripheralManagerStatePoweredOn:
            CSCLog(@">>>CBPeripheralManagerStatePoweredOn");
...
//    if([CSCSpeaker callback] blockOnPeripheralModelDidUpdateState) {
