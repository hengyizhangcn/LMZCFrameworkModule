...
#import "CSCRefreshAutoFooter.h"
#import "UIView+CSCRefreshExtension.h"

@interface CSCRefreshAutoFooter()
/** 一个新的拖拽 */
@property (assign, nonatomic, getter=isOneNewPan) BOOL oneNewPan;
@end

@implementation CSCRefreshAutoFooter
...
- (void)scrollViewContentOffsetDidChange:(NSDictionary *)change
{
    ...
    if (self.state != CSCRefreshStateIdle || !self.automaticallyRefresh || self.mj_y == 0) return;
...
- (void)scrollViewPanStateDidChange:(NSDictionary *)change
{
    ...
    if (self.state != CSCRefreshStateIdle) return;
...
- (void)setState:(CSCRefreshState)state
{
    ...
    if (state == CSCRefreshStateRefreshing) {
        [self executeRefreshingCallback];
    } else if (state == CSCRefreshStateNoMoreData || state == CSCRefreshStateIdle) {
