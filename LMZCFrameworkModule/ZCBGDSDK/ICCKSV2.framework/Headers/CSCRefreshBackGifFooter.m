...
#import "CSCRefreshBackGifFooter.h"
#import "UIView+CSCRefreshExtension.h"

@interface CSCRefreshBackGifFooter()
{
...
@implementation CSCRefreshBackGifFooter
#pragma mark - 懒加载
- (UIImageView *)gifView
{
    if (!_gifView) {
...
- (void)setPullingPercent:(CGFloat)pullingPercent
{
    ...
    NSArray *images = self.stateImages[@(CSCRefreshStateIdle)];
    if (self.state != CSCRefreshStateIdle || images.count == 0) return;
...
- (void)setState:(CSCRefreshState)state
{
    ...
    // 根据状态做事情
    if (state == CSCRefreshStatePulling || state == CSCRefreshStateRefreshing) {
    ...
    } else if (state == CSCRefreshStateIdle) {
        self.gifView.hidden = NO;
