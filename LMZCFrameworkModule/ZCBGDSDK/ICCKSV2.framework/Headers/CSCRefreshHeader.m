...
@interface CSCRefreshHeader()
@property (assign, nonatomic) CGFloat insetTDelta;
@end

@implementation CSCRefreshHeader
#pragma mark - 构造方法
+ (instancetype)headerWithRefreshingBlock:(CSCRefreshComponentRefreshingBlock)refreshingBlock
{
    CSCRefreshHeader *cmp = [[self alloc] init];
...
- (void)prepare
{
    ...
    // 设置key
    self.lastUpdatedTimeKey = CSCRefreshHeaderLastUpdatedTimeKey;
    
    // 设置高度
    self.mj_h = CSCRefreshHeaderHeight;
}
...
- (void)scrollViewContentOffsetDidChange:(NSDictionary *)change
{
    ...
    if (self.scrollView.isDragging) { // 如果正在拖拽
        ...
        if (self.state == CSCRefreshStateIdle && offsetY < normal2pullingOffsetY) {
            // 转为即将刷新状态
            self.state = CSCRefreshStatePulling;
        } else if (self.state == CSCRefreshStatePulling && offsetY >= normal2pullingOffsetY) {
            // 转为普通状态
...
- (void)setState:(CSCRefreshState)state
{
    ...
    if (state == CSCRefreshStateIdle) {
        if (oldState != CSCRefreshStateRefreshing) return;
