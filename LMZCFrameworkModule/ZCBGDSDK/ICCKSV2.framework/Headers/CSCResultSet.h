...
/** Represents the results of executing a query on an `<CSCDatabase>`.
...
@interface CSCResultSet : NSObject

@property (nonatomic, retain, nullable) CSCDatabase *parentDB;
...
/** `CSCStatement` used by result set. */

@property (atomic, retain, nullable) CSCStatement *statement;

///------------------------------------
/// @name Creating and closing a result set
///------------------------------------

/** Create result set from `<CSCStatement>`
 
 @param statement A `<CSCStatement>` to be performed
 
 @param aDB A `<CSCDatabase>` to be used
 
 @return A `CSCResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(CSCStatement *)statement usingParentDatabase:(CSCDatabase*)aDB;
