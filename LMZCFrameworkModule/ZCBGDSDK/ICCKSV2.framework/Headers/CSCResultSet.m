...
+ (instancetype)resultSetWithStatement:(CSCStatement *)statement usingParentDatabase:(CSCDatabase*)aDB {
    ...
    return CSCDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    CSCDBRelease(_columnNameToIndexMap);
...
- (NSDictionary *)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return CSCDBReturnAutoreleased([dict copy]);
    }
    else {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"CSCDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
