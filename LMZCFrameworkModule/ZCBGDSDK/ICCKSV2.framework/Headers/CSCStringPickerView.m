...
#import "CSCStringPickerView.h"
#import "CSCPickerViewMacro.h"

typedef NS_ENUM(NSInteger, CSCStringPickerMode) {
    CSCStringPickerComponentSingle,  // 单列
    CSCStringPickerComponentMore     // 多列
};

@interface CSCStringPickerView ()<UIPickerViewDelegate,UIPickerViewDataSource>
{
...
@property (nonatomic, assign) CSCStringPickerMode type;
...
@property (nonatomic, copy) CSCStringResultBlock resultBlock;
@property (nonatomic, copy) CSCStringCancelBlock cancelBlock;

@end

@implementation CSCStringPickerView

#pragma mark - 1.显示自定义字符串选择器
+ (void)showStringPickerWithTitle:(NSString *)title
                       dataSource:(id)dataSource
                  defaultSelValue:(id)defaultSelValue
                      resultBlock:(CSCStringResultBlock)resultBlock {
...
+ (void)showStringPickerWithTitle:(NSString *)title
                  ...
                  defaultSelValue:(id)defaultSelValue
                     isAutoSelect:(BOOL)isAutoSelect
                       themeColor:(UIColor *)themeColor
                      resultBlock:(CSCStringResultBlock)resultBlock
                      cancelBlock:(CSCStringCancelBlock)cancelBlock {
    CSCStringPickerView *strPickerView = [[CSCStringPickerView alloc]initWithTitle:title dataSource:dataSource defaultSelValue:defaultSelValue isAutoSelect:isAutoSelect themeColor:themeColor resultBlock:resultBlock cancelBlock:cancelBlock];
...
- (void)configDataSource:(id)dataSource defaultSelValue:(id)defaultSelValue {
    ...
    if ([[self.dataSourceArr firstObject] isKindOfClass:[NSString class]]) {
        self.type = CSCStringPickerComponentSingle;
    } else if ([[self.dataSourceArr firstObject] isKindOfClass:[NSArray class]]) {
        self.type = CSCStringPickerComponentMore;
    }
    // 4. 给选择器设置默认值
    if (self.type == CSCStringPickerComponentSingle) {
    ...
    } else if (self.type == CSCStringPickerComponentMore) {
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView {
    switch (self.type) {
        case CSCStringPickerComponentSingle:
            return 1;
            break;
        case CSCStringPickerComponentMore:
