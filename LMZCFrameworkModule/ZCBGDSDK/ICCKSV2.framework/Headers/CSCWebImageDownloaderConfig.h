...
typedef NS_ENUM(NSInteger, CSCWebImageDownloaderExecutionOrder) {
    /**
     * Default value. All download operations will execute in queue style (first-in-first-out).
     */
    CSCWebImageDownloaderFIFOExecutionOrder,
    
    /**
     * All download operations will execute in stack style (last-in-first-out).
     */
    CSCWebImageDownloaderLIFOExecutionOrder
};
 ...
 */
@interface CSCWebImageDownloaderConfig : NSObject <NSCopying>

/**
 Gets the default downloader config used for shared instance or initialization when it does not provide any downloader config. Such as `CSCWebImageDownloader.sharedDownloader`.
 ...
 */
@property (nonatomic, class, readonly, nonnull) CSCWebImageDownloaderConfig *defaultDownloaderConfig;
...
/**
 * Gets/Sets a subclass of `CSCWebImageDownloaderOperation` as the default
 * `NSOperation` to be used each time CSCWebImage constructs a request
 * operation to download an image.
 * Defaults to nil.
 * @note Passing `NSOperation<CSCWebImageDownloaderOperation>` to set as default. Passing `nil` will revert to `CSCWebImageDownloaderOperation`.
 */
@property (nonatomic, assign, nullable) Class operationClass;

/**
 * Changes download operations execution order.
 * Defaults to `CSCWebImageDownloaderFIFOExecutionOrder`.
 */
@property (nonatomic, assign) CSCWebImageDownloaderExecutionOrder executionOrder;
