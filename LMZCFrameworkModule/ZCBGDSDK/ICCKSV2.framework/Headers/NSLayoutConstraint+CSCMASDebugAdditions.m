//
//  NSLayoutConstraint+CSCMASDebugAdditions.m
//  Masonry
//
...
#import "NSLayoutConstraint+CSCMASDebugAdditions.h"
#import "CSCMASConstraint.h"
#import "CSCMASLayoutConstraint.h"

@implementation NSLayoutConstraint (CSCMASDebugAdditions)
...
+ (NSDictionary *)layoutPriorityDescriptionsByValue {
    ...
    dispatch_once(&once, ^{
        ...
        descriptionMap = @{
            @(CSCMASLayoutPriorityDefaultHigh)      : @"high",
            @(CSCMASLayoutPriorityDefaultLow)       : @"low",
            @(CSCMASLayoutPriorityDefaultMedium)    : @"medium",
            @(CSCMASLayoutPriorityRequired)         : @"required",
            @(CSCMASLayoutPriorityFittingSizeLevel) : @"fitting size",
        };
#elif TARGET_OS_MAC
        descriptionMap = @{
            @(CSCMASLayoutPriorityDefaultHigh)                 : @"high",
            @(CSCMASLayoutPriorityDragThatCanResizeWindow)     : @"drag can resize window",
            @(CSCMASLayoutPriorityDefaultMedium)               : @"medium",
            @(CSCMASLayoutPriorityWindowSizeStayPut)           : @"window size stay put",
            @(CSCMASLayoutPriorityDragThatCannotResizeWindow)  : @"drag cannot resize window",
            @(CSCMASLayoutPriorityDefaultLow)                  : @"low",
            @(CSCMASLayoutPriorityFittingSizeCompression)      : @"fitting size",
            @(CSCMASLayoutPriorityRequired)                    : @"required",
        };
#endif
    });
...
- (NSString *)description {
    ...
    if (self.priority != CSCMASLayoutPriorityRequired) {
