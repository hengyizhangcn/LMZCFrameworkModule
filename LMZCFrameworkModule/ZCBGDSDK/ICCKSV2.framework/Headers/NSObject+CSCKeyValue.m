...
#import "NSObject+CSCKeyValue.h"
#import "NSObject+CSCProperty.h"
#import "NSString+CSCExtension.h"
#import "CSCProperty.h"
#import "CSCPropertyType.h"
#import "CSCExtensionConst.h"
#import "CSCFoundation.h"
#import "NSString+CSCExtension.h"
#import "NSObject+CSCClass.h"
...
- (instancetype)mj_setKeyValues:(id)keyValues context:(NSManagedObjectContext *)context
{
    ...
    [clazz mj_enumerateProperties:^(CSCProperty *property, BOOL *stop) {
        @try {
            // 0.检测是否被忽略
            ...
            for (NSArray *propertyKeys in propertyKeyses) {
                value = keyValues;
                for (CSCPropertyKey *propertyKey in propertyKeys) {
            ...
            // 2.复杂处理
            CSCPropertyType *type = property.type;
...
+ (NSMutableArray *)mj_objectArrayWithKeyValuesArray:(id)keyValuesArray context:(NSManagedObjectContext *)context
{
    ...
    if ([CSCFoundation isClassFromFoundation:self]) return [NSMutableArray arrayWithArray:keyValuesArray];
...
- (NSMutableDictionary *)mj_keyValuesWithKeys:(NSArray *)keys ignoredKeys:(NSArray *)ignoredKeys
{
    ...
    // TODO: 报错机制不好, 需要重做
    MJExtensionAssertError(![CSCFoundation isClassFromFoundation:[self class]], (NSMutableDictionary *)self, [self class], @"不是自定义的模型类")
    ...
    [clazz mj_enumerateProperties:^(CSCProperty *property, BOOL *stop) {
        @try {
            ...
            if ([clazz mj_isReferenceReplacedKeyWhenCreatingKeyValues]) {
                ...
                [propertyKeys enumerateObjectsUsingBlock:^(CSCPropertyKey *propertyKey, NSUInteger idx, BOOL *stop) {
                    // 下一个属性
                    CSCPropertyKey *nextPropertyKey = nil;
                    if (idx != keyCount - 1) {
                    ...
                    if (nextPropertyKey) { // 不是最后一个key
                        ...
                        if (tempInnerContainer == nil || [tempInnerContainer isKindOfClass:[NSNull class]]) {
                            if (nextPropertyKey.type == CSCPropertyKeyTypeDictionary) {
                                tempInnerContainer = [NSMutableDictionary dictionary];
                            } else {
                                tempInnerContainer = [NSMutableArray array];
                            }
                            if (propertyKey.type == CSCPropertyKeyTypeDictionary) {
