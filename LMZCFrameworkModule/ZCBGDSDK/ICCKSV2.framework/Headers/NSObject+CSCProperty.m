...
+ (void)mj_enumerateProperties:(MJPropertiesEnumeration)enumeration
{
    ...
    // 遍历成员变量
    BOOL stop = NO;
    for (CSCProperty *property in cachedProperties) {
        ...
        if (cachedProperties == nil) {
            ...
            [self mj_enumerateClasses:^(__unsafe_unretained Class c, BOOL *stop) {
                ...
                for (unsigned int i = 0; i<outCount; i++) {
                    CSCProperty *property = [CSCProperty cachedPropertyWithProperty:properties[i]];
                    // 过滤掉Foundation框架类里面的属性
                    if ([CSCFoundation isClassFromFoundation:property.srcClass]) continue;
                    // 过滤掉`hash`, `superclass`, `description`, `debugDescription`
                    if ([CSCFoundation isFromNSObjectProtocolProperty:property.name]) continue;
...
+ (id)mj_getNewValueFromObject:(__unsafe_unretained id)object oldValue:(__unsafe_unretained id)oldValue property:(CSCProperty *__unsafe_unretained)property{
    // 如果有实现方法
