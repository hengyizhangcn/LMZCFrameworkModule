...
static inline CGRect CSCCGRectFitWithScaleMode(CGRect rect, CGSize size, CSCImageScaleMode scaleMode) {
    rect = CGRectStandardize(rect);
    size.width = size.width < 0 ? -size.width : size.width;
    size.height = size.height < 0 ? -size.height : size.height;
    CGPoint center = CGPointMake(CGRectGetMidX(rect), CGRectGetMidY(rect));
    switch (scaleMode) {
        case CSCImageScaleModeAspectFit:
        case CSCImageScaleModeAspectFill: {
            if (rect.size.width < 0.01 || rect.size.height < 0.01 ||
                ...
                CGFloat scale;
                if (scaleMode == CSCImageScaleModeAspectFit) {
...
static inline UIColor * CSCGetColorFromPixel(Pixel_8888 pixel, CGBitmapInfo bitmapInfo) {
...
- (void)sd_drawInRect:(CGRect)rect withScaleMode:(CSCImageScaleMode)scaleMode clipsToBounds:(BOOL)clips {
    CGRect drawRect = CSCCGRectFitWithScaleMode(rect, self.size, scaleMode);
    if (drawRect.size.width == 0 || drawRect.size.height == 0) return;
    if (clips) {
        CGContextRef context = CSCGraphicsGetCurrentContext();
        if (context) {
...
- (nullable UIImage *)sd_resizedImageWithSize:(CGSize)size scaleMode:(CSCImageScaleMode)scaleMode {
    if (size.width <= 0 || size.height <= 0) return nil;
    CSCGraphicsBeginImageContextWithOptions(size, NO, self.scale);
    [self sd_drawInRect:CGRectMake(0, 0, size.width, size.height) withScaleMode:scaleMode clipsToBounds:NO];
    UIImage *image = CSCGraphicsGetImageFromCurrentImageContext();
    CSCGraphicsEndImageContext();
    return image;
}
...
- (nullable UIImage *)sd_roundedCornerImageWithRadius:(CGFloat)cornerRadius corners:(CSCRectCorner)corners borderWidth:(CGFloat)borderWidth borderColor:(nullable UIColor *)borderColor {
    if (!self.CGImage) return nil;
    CSCGraphicsBeginImageContextWithOptions(self.size, NO, self.scale);
    CGContextRef context = CSCGraphicsGetCurrentContext();
...
- (nullable UIImage *)sd_tintedImageWithColor:(nonnull UIColor *)tintColor {
    ...
    CSCGraphicsBeginImageContextWithOptions(size, NO, scale);
    CGContextRef context = CSCGraphicsGetCurrentContext();
    [self drawInRect:rect];
...
- (nullable UIColor *)sd_colorAtPoint:(CGPoint)point {
    ...
    return CSCGetColorFromPixel(pixel, bitmapInfo);
}
...
- (nullable NSArray<UIColor *> *)sd_colorsWithRect:(CGRect)rect {
    ...
    for (size_t index = start; index < end; index += 4) {
        ...
        UIColor *color = CSCGetColorFromPixel(pixel, bitmapInfo);
...
- (nullable UIImage *)sd_blurredImageWithRadius:(CGFloat)blurRadius {
    ...
    if (err != kvImageNoError) {
        NSLog(@"UIImage+CSCTransform error: vImageBuffer_InitWithCGImage returned error code %zi for inputImage: %@", err, self);
        return nil;
    }
    err = vImageBuffer_Init(&scratch, effect.height, effect.width, format.bitsPerPixel, kvImageNoFlags);
    if (err != kvImageNoError) {
        NSLog(@"UIImage+CSCTransform error: vImageBuffer_Init returned error code %zi for inputImage: %@", err, self);
        return nil;
    }
