 ...
 */
FOUNDATION_EXPORT const int64_t CSCWebImageProgressUnitCountUnknown; /* 1LL */

typedef void(^CSCSetImageBlock)(UIImage * _Nullable image, NSData * _Nullable imageData, CSCImageCacheType cacheType, NSURL * _Nullable imageURL);

/**
 Integrates CSCWebImage async downloading and caching of remote images with UIView subclass.
 */
 ...
 * The `totalUnitCount` and `completedUnitCount` will be reset to 0 after a new image loading start (change from current queue). And they will be set to `CSCWebImageProgressUnitCountUnknown` if the progressBlock not been called but the image loading success to mark the progress finished (change from main queue).
 ...
 * @param options        The options to use when downloading the image. @see CSCWebImageOptions for the possible values.
 ...
 *
 *   The fith parameter normally is always YES. However, if you provide CSCWebImageAvoidAutoSetImage with CSCWebImageProgressiveLoad options to enable progressive downloading and set the image yourself. This block is thus called repeatedly with a partial image. When image is fully downloaded, the
...
- (void)sd_internalSetImageWithURL:(nullable NSURL *)url
                  placeholderImage:(nullable UIImage *)placeholder
                     ...
                     setImageBlock:(nullable CSCSetImageBlock)setImageBlock
...
/**
 The image transition when image load finished. See `CSCWebImageTransition`.
 If you specify nil, do not do transition. Defautls to nil.
 */
@property (nonatomic, strong, nullable) CSCWebImageTransition *sd_imageTransition;
 ...
 */
@property (nonatomic, strong, nullable) id<CSCWebImageIndicator> sd_imageIndicator;
